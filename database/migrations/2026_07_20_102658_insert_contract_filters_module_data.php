...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        if (DB::table('m_modules')->where('identifier', 'contract_filter_templates')->exists()) {
            return;
        }

        // Resolve module group dynamically — look up by name to avoid hardcoded UUID issues
        $moduleGroupNames = ['Pengaturan Sistem', 'Master Data', 'Lainnya'];
        $moduleGroup = null;
        ...
        DB::table('m_modules')->insert([
            'id' => $moduleId,
            'name' => 'Konfigurasi Filter Kontrak',
            'identifier' => 'contract_filter_templates',
            'module_group_id' => $moduleGroupId,
            'icon' => 'ShieldAlert',
            'route' => '/admin/core/contract-filter-templates',
            'showed_as_menu' => true,
            'is_active' => true,
            'created_at' => now(),
            'updated_at' => now(),
    ...
    {
        $module = DB::table('m_modules')->where('identifier', 'contract_filter_templates')->first();
        if ($module) {
