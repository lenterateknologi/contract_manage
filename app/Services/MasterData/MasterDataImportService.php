<?php

declare(strict_types=1);

namespace App\Services\MasterData;

use App\Models\AccessModule;
use App\Models\Company;
use App\Models\CompanyGroup;
use App\Models\ContractStatus;
use App\Models\ContractType;
use App\Models\Department;
use App\Models\Division;
use App\Models\FormField;
use App\Models\FormTemplate;
use App\Models\Module;
use App\Models\ModuleGroup;
use App\Models\Region;
use App\Models\Role;
use App\Models\RoleModuleGroup;
use App\Models\User;
use App\Models\Authority;
use App\Models\Workflow;
use App\Models\WorkflowStep;
use App\Models\WorkflowStepAction;
use Illuminate\Database\Eloquent\Model;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Log;
use Illuminate\Support\Str;

class MasterDataImportService
{
    private array $counts = [];
    private ?string $adminId = null;

    // ID translation maps
    private array $roleMap = [];
    private array $roleIdMap = [];
    private array $moduleMap = [];
    private array $moduleGroupMap = [];
    private array $groupMap = [];
    private array $companyGroupIdMap = [];
    private array $regionMap = [];
    private array $regionIdMap = [];
    private array $companyMap = [];
    private array $companyIdMap = [];
    private array $deptMap = [];
    private array $departmentIdMap = [];
    private array $divisionMap = [];
    private array $divisionIdMap = [];
    private array $workflowIdMap = [];
    private array $workflowStepIdMap = [];

    /**
     * Parse and process import of master data array.
     */
    public function execute(array $data): array
    {
        $this->counts = [
            'company_groups' => 0,
            'regions' => 0,
            'companies' => 0,
            'departments' => 0,
            'divisions' => 0,
            'contract_statuses' => 0,
            'contract_types' => 0,
            'workflows' => 0,
            'workflow_steps' => 0,
            'workflow_step_authorities' => 0,
            'workflow_initiator_authorities' => 0,
            'workflow_step_actions' => 0,
            'roles' => 0,
            'access_mappings' => 0,
            'role_navigation_mappings' => 0,
            'form_templates' => 0,
            'form_fields' => 0,
            'users' => 0,
        ];

        Model::unguard();
        $this->adminId = Auth::id();

        // 1. Roles, Modules & Navigation
        $this->importRoles($data);
        $this->importModuleGroups($data);
        $this->importModules($data);

        // 2. Organization Hierarchy
        $this->importCompanyGroups($data);
        $this->importRegions($data);
        $this->importCompanies($data);
        $this->importDepartments($data);
        $this->importDivisions($data);

        // 3. Contract Statuses
        $this->importContractStatuses($data);

        // 4. Workflows & Steps
        $this->importWorkflows($data);
        $this->importWorkflowInitiatorAuthorities($data);
        $this->importWorkflowSteps($data);
        $this->importWorkflowStepAuthorities($data);
        $this->importWorkflowStepActions($data);

        // 5. Form Templates & Fields
        $this->importFormTemplatesAndFields($data);

        // 6. Contract Types
        $this->importContractTypes($data);

        // 7. Access & Role Navigation Mappings
        $this->importAccessMappings($data);
        $this->importRoleNavigationMappings($data);

        // 8. Users
        $this->importUsers($data);

        return $this->counts;
    }

    private function importRoles(array $data): void
    {
        if (empty($data['roles']) || ! is_array($data['roles'])) {
            return;
        }

        foreach ($data['roles'] as $r) {
            try {
                if (empty($r['name'])) {
                    continue;
                }
                Role::withTrashed()->updateOrCreate(
                    ! empty($r['id']) ? ['id' => $r['id']] : ['name' => $r['name']],
                    [
                        'name' => $r['name'],
                        'description' => $r['description'] ?? null,
                    ],
                );
                $this->counts['roles']++;
            } catch (\Exception $e) {
                Log::warning('Gagal mengimpor Role '.($r['name'] ?? '').': '.$e->getMessage());
            }
        }

        $this->roleMap = Role::pluck('id', 'name')->all();
        $this->roleIdMap = [];
        foreach ($data['roles'] as $r) {
            if (! empty($r['id']) && ! empty($r['name'])) {
                $this->roleIdMap[$r['id']] = $this->roleMap[$r['name']] ?? null;
            }
        }
    }

    private function importModuleGroups(array $data): void
    {
        $this->moduleGroupMap = ModuleGroup::pluck('id', 'name')->all();

        if (empty($data['module_groups']) || ! is_array($data['module_groups'])) {
            return;
        }

        foreach ($data['module_groups'] as $mg) {
            try {
                if (empty($mg['name'])) {
                    continue;
                }
                $existingGroup = ModuleGroup::withTrashed()->where('name', $mg['name'])->first();
                if ($existingGroup) {
                    $existingGroup->update([
                        'icon' => $mg['icon'] ?? config('master.module_group.default_icon'),
                        'updated_by' => $this->adminId,
                        'deleted_at' => null,
                    ]);
                } else {
                    ModuleGroup::create([
                        'id' => $mg['id'] ?? (string) Str::uuid(),
                        'name' => $mg['name'],
                        'icon' => $mg['icon'] ?? config('master.module_group.default_icon'),
                        'deleted_at' => null,
                        'created_by' => $this->adminId,
                        'updated_by' => $this->adminId,
                    ]);
                }
            } catch (\Exception $e) {
                Log::warning('Gagal mengimpor ModuleGroup '.($mg['name'] ?? '').': '.$e->getMessage());
            }
        }
        $this->moduleGroupMap = ModuleGroup::pluck('id', 'name')->all();
    }

    private function importModules(array $data): void
    {
        $this->moduleMap = Module::pluck('id', 'identifier')->all();

        if (empty($data['modules']) || ! is_array($data['modules'])) {
            return;
        }

        foreach ($data['modules'] as $m) {
            try {
                if (empty($m['identifier'])) {
                    continue;
                }
                $groupId = ! empty($m['module_group_name']) ? ($this->moduleGroupMap[$m['module_group_name']] ?? null) : null;
                Module::withTrashed()->updateOrCreate(
                    ! empty($m['id']) ? ['id' => $m['id']] : ['identifier' => $m['identifier']],
                    [
                        'identifier' => $m['identifier'],
                        'name' => $m['name'],
                        'route' => $m['route'] ?? null,
                        'icon' => $m['icon'] ?? null,
                        'description' => $m['description'] ?? null,
                        'module_group_id' => $groupId,
                        'showed_as_menu' => $m['showed_as_menu'] ?? true,
                        'deleted_at' => null,
                        'created_by' => $this->adminId,
                        'updated_by' => $this->adminId,
                    ],
                );
            } catch (\Exception $e) {
                Log::warning('Gagal mengimpor Module '.($m['name'] ?? '').': '.$e->getMessage());
            }
        }
        $this->moduleMap = Module::pluck('id', 'identifier')->all();
    }

    private function importCompanyGroups(array $data): void
    {
        if (empty($data['company_groups']) || ! is_array($data['company_groups'])) {
            return;
        }

        foreach ($data['company_groups'] as $g) {
            try {
                if (empty($g['code'])) {
                    continue;
                }
                CompanyGroup::withTrashed()->updateOrCreate(
                    ! empty($g['id']) ? ['id' => $g['id']] : ['code' => $g['code']],
                    [
                        'code' => $g['code'],
                        'name' => $g['name'] ?? $g['code'],
                        'description' => $g['description'] ?? null,
                        'is_active' => $g['is_active'] ?? true,
                        'deleted_at' => null,
                        'created_by' => $this->adminId,
                        'updated_by' => $this->adminId,
                    ],
                );
                $this->counts['company_groups']++;
            } catch (\Exception $e) {
                Log::warning('Gagal mengimpor CompanyGroup '.($g['code'] ?? '').': '.$e->getMessage());
            }
        }

        $this->groupMap = CompanyGroup::pluck('id', 'code')->all();
        $this->companyGroupIdMap = [];
        foreach ($data['company_groups'] as $g) {
            if (! empty($g['id']) && ! empty($g['code'])) {
                $this->companyGroupIdMap[$g['id']] = $this->groupMap[$g['code']] ?? null;
            }
        }
    }

    private function importRegions(array $data): void
    {
        if (empty($data['regions']) || ! is_array($data['regions'])) {
            return;
        }

        foreach ($data['regions'] as $r) {
            try {
                if (empty($r['code'])) {
                    continue;
                }
                Region::withTrashed()->updateOrCreate(
                    ! empty($r['id']) ? ['id' => $r['id']] : ['code' => $r['code']],
                    [
                        'code' => $r['code'],
                        'name' => $r['name'] ?? $r['code'],
                        'alias' => $r['alias'] ?? null,
                        'description' => $r['description'] ?? null,
                        'is_active' => $r['is_active'] ?? true,
                        'id_portal_master' => $r['id_portal_master'] ?? null,
                        'deleted_at' => null,
                        'created_by' => $this->adminId,
                        'updated_by' => $this->adminId,
                    ],
                );
                $this->counts['regions']++;
            } catch (\Exception $e) {
                Log::warning('Gagal mengimpor Region '.($r['code'] ?? '').': '.$e->getMessage());
            }
        }

        $this->regionMap = Region::pluck('id', 'code')->all();
        $this->regionIdMap = [];
        foreach ($data['regions'] as $r) {
            if (! empty($r['id']) && ! empty($r['code'])) {
                $this->regionIdMap[$r['id']] = $this->regionMap[$r['code']] ?? null;
            }
        }
    }

    private function importCompanies(array $data): void
    {
        if (empty($data['companies']) || ! is_array($data['companies'])) {
            return;
        }

        foreach ($data['companies'] as $c) {
            try {
                if (empty($c['code'])) {
                    continue;
                }
                $groupId = ! empty($c['company_group_code']) ? ($this->groupMap[$c['company_group_code']] ?? null) : null;
                $regionId = ! empty($c['region_code']) ? ($this->regionMap[$c['region_code']] ?? null) : null;

                Company::withTrashed()->updateOrCreate(
                    ! empty($c['id']) ? ['id' => $c['id']] : ['code' => $c['code']],
                    [
                        'code' => $c['code'],
                        'name' => $c['name'] ?? $c['code'],
                        'alias' => $c['alias'] ?? null,
                        'address' => $c['address'] ?? null,
                        'company_group_id' => $groupId,
                        'region_id' => $regionId,
                        'is_active' => $c['is_active'] ?? true,
                        'deleted_at' => null,
                        'created_by' => $this->adminId,
                        'updated_by' => $this->adminId,
                    ],
                );
                $this->counts['companies']++;
            } catch (\Exception $e) {
                Log::warning('Gagal mengimpor Company '.($c['code'] ?? '').': '.$e->getMessage());
            }
        }

        $this->companyMap = Company::pluck('id', 'code')->all();
        $this->companyIdMap = [];
        foreach ($data['companies'] as $c) {
            if (! empty($c['id']) && ! empty($c['code'])) {
                $this->companyIdMap[$c['id']] = $this->companyMap[$c['code']] ?? null;
            }
        }
    }

    private function importDepartments(array $data): void
    {
        if (empty($data['departments']) || ! is_array($data['departments'])) {
            return;
        }

        foreach ($data['departments'] as $d) {
            try {
                if (empty($d['code'])) {
                    continue;
                }
                $companyId = ! empty($d['company_code']) ? ($this->companyMap[$d['company_code']] ?? null) : null;

                Department::withTrashed()->updateOrCreate(
                    ! empty($d['id']) ? ['id' => $d['id']] : ['code' => $d['code']],
                    [
                        'code' => $d['code'],
                        'name' => $d['name'] ?? $d['code'],
                        'description' => $d['description'] ?? null,
                        'company_id' => $companyId,
                        'is_active' => $d['is_active'] ?? true,
                        'deleted_at' => null,
                        'created_by' => $this->adminId,
                        'updated_by' => $this->adminId,
                    ],
                );
                $this->counts['departments']++;
            } catch (\Exception $e) {
                Log::warning('Gagal mengimpor Department '.($d['code'] ?? '').': '.$e->getMessage());
            }
        }

        $this->deptMap = Department::pluck('id', 'code')->all();
        $this->departmentIdMap = [];
        foreach ($data['departments'] as $d) {
            if (! empty($d['id']) && ! empty($d['code'])) {
                $this->departmentIdMap[$d['id']] = $this->deptMap[$d['code']] ?? null;
            }
        }
    }

    private function importDivisions(array $data): void
    {
        if (empty($data['divisions']) || ! is_array($data['divisions'])) {
            return;
        }

        foreach ($data['divisions'] as $d) {
            try {
                if (empty($d['code']) && empty($d['name'])) {
                    continue;
                }
                $deptId = ! empty($d['department_code']) ? ($this->deptMap[$d['department_code']] ?? null) : null;

                Division::withTrashed()->updateOrCreate(
                    ! empty($d['id']) ? ['id' => $d['id']] : ['code' => $d['code']],
                    [
                        'code' => $d['code'] ?? null,
                        'name' => $d['name'],
                        'department_id' => $deptId,
                        'id_portal_master' => $d['id_portal_master'] ?? null,
                        'is_active' => $d['is_active'] ?? true,
                        'deleted_at' => null,
                        'created_by' => $this->adminId,
                        'updated_by' => $this->adminId,
                    ],
                );
                $this->counts['divisions']++;
            } catch (\Exception $e) {
                Log::warning('Gagal mengimpor Division '.($d['name'] ?? '').': '.$e->getMessage());
            }
        }

        $this->divisionMap = Division::pluck('id', 'code')->all();
        $this->divisionIdMap = [];
        foreach ($data['divisions'] as $d) {
            if (! empty($d['id']) && ! empty($d['code'])) {
                $this->divisionIdMap[$d['id']] = $this->divisionMap[$d['code']] ?? null;
            }
        }
    }

    private function importContractStatuses(array $data): void
    {
        if (empty($data['contract_statuses']) || ! is_array($data['contract_statuses'])) {
            return;
        }

        foreach ($data['contract_statuses'] as $s) {
            try {
                if (empty($s['code'])) {
                    continue;
                }
                ContractStatus::withTrashed()->updateOrCreate(
                    ! empty($s['id']) ? ['id' => $s['id']] : ['code' => $s['code']],
                    [
                        'code' => $s['code'],
                        'label' => $s['label'] ?? $s['code'],
                        'color' => $s['color'] ?? null,
                        'bg_color' => $s['bg_color'] ?? null,
                        'icon' => $s['icon'] ?? null,
                        'description' => $s['description'] ?? null,
                        'is_active' => $s['is_active'] ?? true,
                        'deleted_at' => null,
                        'created_by' => $this->adminId,
                        'updated_by' => $this->adminId,
                    ],
                );
                $this->counts['contract_statuses']++;
            } catch (\Exception $e) {
                Log::warning('Gagal mengimpor ContractStatus '.($s['code'] ?? '').': '.$e->getMessage());
            }
        }
    }

    private function importWorkflows(array $data): void
    {
        $this->workflowIdMap = [];
        if (empty($data['workflows']) || ! is_array($data['workflows'])) {
            return;
        }

        foreach ($data['workflows'] as $w) {
            try {
                if (empty($w['id'])) {
                    continue;
                }

                $w['department_id'] = ! empty($w['department_id']) ? ($this->departmentIdMap[$w['department_id']] ?? $w['department_id']) : null;

                if (! empty($w['company_group_ids'])) {
                    $oldIds = is_array($w['company_group_ids']) ? $w['company_group_ids'] : json_decode($w['company_group_ids'], true);
                    if (is_array($oldIds)) {
                        $newIds = array_map(fn ($id) => $this->companyGroupIdMap[$id] ?? $id, $oldIds);
                        $w['company_group_ids'] = json_encode($newIds);
                    }
                }

                if (! empty($w['region_ids'])) {
                    $oldIds = is_array($w['region_ids']) ? $w['region_ids'] : json_decode($w['region_ids'], true);
                    if (is_array($oldIds)) {
                        $newIds = array_map(fn ($id) => $this->regionIdMap[$id] ?? $id, $oldIds);
                        $w['region_ids'] = json_encode($newIds);
                    }
                }

                if (! empty($w['company_ids'])) {
                    $oldIds = is_array($w['company_ids']) ? $w['company_ids'] : json_decode($w['company_ids'], true);
                    if (is_array($oldIds)) {
                        $newIds = array_map(fn ($id) => $this->companyIdMap[$id] ?? $id, $oldIds);
                        $w['company_ids'] = json_encode($newIds);
                    }
                }

                if (array_key_exists('contract_type', $w)) {
                    $contractTypeVal = $w['contract_type'];
                    unset($w['contract_type']);
                    if ($contractTypeVal) {
                        $w['contract_type_id'] = ContractType::where('code', $contractTypeVal)
                            ->orWhere('name', $contractTypeVal)
                            ->value('id');
                    } else {
                        $w['contract_type_id'] = null;
                    }
                }

                $dbW = Workflow::withTrashed()->updateOrCreate(
                    ['id' => $w['id']],
                    [
                        'name' => $w['name'],
                        'description' => $w['description'] ?? null,
                        'is_default' => $w['is_default'] ?? false,
                        'is_template' => $w['is_template'] ?? false,
                        'is_tax_involved' => $w['is_tax_involved'] ?? false,
                        'initiator_type' => $w['initiator_type'] ?? config('master.workflow.initiator_type'),
                        'sla_drafting_hours' => $w['sla_drafting_hours'] ?? config('master.workflow.sla_drafting_hours'),
                        'sla_total_hours' => $w['sla_total_hours'] ?? config('master.workflow.sla_total_hours'),
                        'sla_cutoff_hour' => $w['sla_cutoff_hour'] ?? config('master.workflow.sla_cutoff_hour'),
                        'scope' => $w['scope'] ?? config('master.workflow.scope'),
                        'workflow_category' => $w['workflow_category'] ?? config('master.workflow.category'),
                        'company_group_ids' => $w['company_group_ids'] ?? null,
                        'region_ids' => $w['region_ids'] ?? null,
                        'company_ids' => $w['company_ids'] ?? null,
                        'department_id' => $w['department_id'] ?? null,
                        'is_active' => $w['is_active'] ?? true,
                        'deleted_at' => null,
                        'created_by' => $this->adminId,
                        'updated_by' => $this->adminId,
                    ],
                );
                $this->workflowIdMap[$w['id']] = $dbW->id;
                $this->counts['workflows']++;
            } catch (\Exception $e) {
                Log::warning('Gagal mengimpor Workflow '.($w['name'] ?? '').': '.$e->getMessage());
            }
        }
    }

    private function importWorkflowInitiatorAuthorities(array $data): void
    {
        if (empty($data['workflow_initiator_authorities']) || ! is_array($data['workflow_initiator_authorities'])) {
            return;
        }

        $userEmailMap = User::pluck('id', 'email')->all();
        foreach ($data['workflow_initiator_authorities'] as $auth) {
            try {
                if (empty($auth['id'])) {
                    continue;
                }
                $wfId = $this->workflowIdMap[$auth['workflow_id']] ?? $auth['workflow_id'];
                $deptId = ! empty($auth['department_id']) ? ($this->departmentIdMap[$auth['department_id']] ?? $auth['department_id']) : null;
                $newUserId = ! empty($auth['user_email']) ? ($userEmailMap[$auth['user_email']] ?? $auth['user_id']) : $auth['user_id'];

                $cgId = ! empty($auth['company_group_id']) ? ($this->companyGroupIdMap[$auth['company_group_id']] ?? $auth['company_group_id']) : null;
                $rgId = ! empty($auth['region_id']) ? ($this->regionIdMap[$auth['region_id']] ?? $auth['region_id']) : null;

                $model = Authority::firstOrNew(['id' => $auth['id']]);
                $model->forceFill([
                    'context_type' => Authority::CONTEXT_WORKFLOW_INITIATOR,
                    'context_id' => $wfId,
                    'role_id' => $auth['role_id'] ?? null,
                    'department_id' => $deptId,
                    'division_id' => $auth['division_id'] ?? null,
                    'user_id' => $newUserId,
                    'authority_type' => $auth['authority_type'] ?? 'group',
                    'company_group_id' => $cgId,
                    'region_id' => $rgId,
                    'role_use_initiator' => $auth['role_use_initiator'] ?? false,
                    'department_use_initiator' => $auth['department_use_initiator'] ?? false,
                    'division_use_initiator' => $auth['division_use_initiator'] ?? false,
                    'company_group_use_initiator' => $auth['company_group_use_initiator'] ?? false,
                    'region_use_initiator' => $auth['region_use_initiator'] ?? false,
                    'is_active' => true,
                ])->save();
                $this->counts['workflow_initiator_authorities']++;
            } catch (\Exception $e) {
                Log::warning('Gagal mengimpor Authority (workflow_initiator) ID '.($auth['id'] ?? '').': '.$e->getMessage());
            }
        }
    }

    private function importWorkflowSteps(array $data): void
    {
        $this->workflowStepIdMap = [];
        if (empty($data['workflow_steps']) || ! is_array($data['workflow_steps'])) {
            return;
        }

        foreach ($data['workflow_steps'] as $s) {
            try {
                if (empty($s['id'])) {
                    continue;
                }

                $workflowId = $this->workflowIdMap[$s['workflow_id']] ?? $s['workflow_id'];
                $roleId = ! empty($s['role_id']) ? ($this->roleIdMap[$s['role_id']] ?? $s['role_id']) : null;

                if (! empty($s['company_group_ids'])) {
                    $oldIds = is_array($s['company_group_ids']) ? $s['company_group_ids'] : json_decode($s['company_group_ids'], true);
                    if (is_array($oldIds)) {
                        $newIds = array_map(fn ($id) => $this->companyGroupIdMap[$id] ?? $id, $oldIds);
                        $s['company_group_ids'] = json_encode($newIds);
                    }
                }
                if (! empty($s['region_ids'])) {
                    $oldIds = is_array($s['region_ids']) ? $s['region_ids'] : json_decode($s['region_ids'], true);
                    if (is_array($oldIds)) {
                        $newIds = array_map(fn ($id) => $this->regionIdMap[$id] ?? $id, $oldIds);
                        $s['region_ids'] = json_encode($newIds);
                    }
                }
                if (! empty($s['company_ids'])) {
                    $oldIds = is_array($s['company_ids']) ? $s['company_ids'] : json_decode($s['company_ids'], true);
                    if (is_array($oldIds)) {
                        $newIds = array_map(fn ($id) => $this->companyIdMap[$id] ?? $id, $oldIds);
                        $s['company_ids'] = json_encode($newIds);
                    }
                }

                $dbStep = WorkflowStep::withTrashed()->updateOrCreate(
                    ! empty($s['id']) ? ['id' => $s['id']] : [
                        'workflow_id' => $workflowId,
                        'step' => $s['step'],
                    ],
                    [
                        'id' => $s['id'],
                        'workflow_id' => $workflowId,
                        'step' => $s['step'],
                        'step_category' => $s['step_category'] ?? null,
                        'approver_type' => $s['approver_type'] ?? config('master.workflow_step.approver_type'),
                        'is_optional' => $s['is_optional'] ?? false,
                        'optional_label' => $s['optional_label'] ?? null,
                        'condition_expression' => $s['condition_expression'] ?? null,
                        'description' => $s['description'] ?? null,
                        'phase' => $s['phase'] ?? config('master.workflow_step.phase'),
                        'uploader_type' => $s['uploader_type'] ?? null,
                        'hierarchy_level' => $s['hierarchy_level'] ?? null,
                        'role_id' => $roleId,
                        'company_group_ids' => $s['company_group_ids'] ?? null,
                        'region_ids' => $s['region_ids'] ?? null,
                        'company_ids' => $s['company_ids'] ?? null,
                        'label' => $s['label'] ?? null,
                        'allowed_actions' => $s['allowed_actions'] ?? null,
                        'is_mandatory' => $s['is_mandatory'] ?? true,
                        'is_active' => $s['is_active'] ?? true,
                        'meta' => $s['meta'] ?? null,
                        'deleted_at' => null,
                        'created_by' => $this->adminId,
                        'updated_by' => $this->adminId,
                    ],
                );
                $this->workflowStepIdMap[$s['id']] = $dbStep->id;
                $this->counts['workflow_steps']++;
            } catch (\Exception $e) {
                Log::warning('Gagal mengimpor WorkflowStep ID '.($s['id'] ?? '').': '.$e->getMessage());
            }
        }
    }

    private function importWorkflowStepAuthorities(array $data): void
    {
        if (empty($data['workflow_step_authorities']) || ! is_array($data['workflow_step_authorities'])) {
            return;
        }

        $userEmailMap = User::pluck('id', 'email')->all();
        foreach ($data['workflow_step_authorities'] as $auth) {
            try {
                if (empty($auth['id'])) {
                    continue;
                }
                $stepId = $this->workflowStepIdMap[$auth['workflow_step_id']] ?? $auth['workflow_step_id'];
                $deptId = ! empty($auth['department_id']) ? ($this->departmentIdMap[$auth['department_id']] ?? $auth['department_id']) : null;
                $divId = ! empty($auth['division_id']) ? ($this->divisionIdMap[$auth['division_id']] ?? $auth['division_id']) : null;
                $newUserId = ! empty($auth['user_email']) ? ($userEmailMap[$auth['user_email']] ?? $auth['user_id']) : $auth['user_id'];

                $wsCgId = ! empty($auth['company_group_id']) ? ($this->companyGroupIdMap[$auth['company_group_id']] ?? $auth['company_group_id']) : null;
                $wsRgId = ! empty($auth['region_id']) ? ($this->regionIdMap[$auth['region_id']] ?? $auth['region_id']) : null;
                $wsActionId = ! empty($auth['workflow_step_action_id']) ? ($this->workflowStepIdMap[$auth['workflow_step_action_id']] ?? $auth['workflow_step_action_id']) : null;
                $wsTargetId = ! empty($auth['target_step_id']) ? ($this->workflowStepIdMap[$auth['target_step_id']] ?? $auth['target_step_id']) : null;

                $model = Authority::firstOrNew(['id' => $auth['id']]);
                $model->forceFill([
                    'context_type' => Authority::CONTEXT_WORKFLOW_STEP,
                    'context_id' => $stepId,
                    'role_id' => $auth['role_id'] ?? null,
                    'department_id' => $deptId,
                    'division_id' => $divId,
                    'user_id' => $newUserId,
                    'authority_type' => $auth['authority_type'] ?? 'group',
                    'is_additional' => $auth['is_additional'] ?? false,
                    'additional_type' => $auth['additional_type'] ?? null,
                    'workflow_step_action_id' => $wsActionId,
                    'target_step_id' => $wsTargetId,
                    'company_group_id' => $wsCgId,
                    'region_id' => $wsRgId,
                    'role_use_initiator' => $auth['role_use_initiator'] ?? false,
                    'department_use_initiator' => $auth['department_use_initiator'] ?? false,
                    'division_use_initiator' => $auth['division_use_initiator'] ?? false,
                    'company_group_use_initiator' => $auth['company_group_use_initiator'] ?? false,
                    'region_use_initiator' => $auth['region_use_initiator'] ?? false,
                    'is_active' => true,
                ])->save();
                $this->counts['workflow_step_authorities']++;
            } catch (\Exception $e) {
                Log::warning('Gagal mengimpor Authority (workflow_step) ID '.($auth['id'] ?? '').': '.$e->getMessage());
            }
        }
    }

    private function importWorkflowStepActions(array $data): void
    {
        if (empty($data['workflow_step_actions']) || ! is_array($data['workflow_step_actions'])) {
            return;
        }

        foreach ($data['workflow_step_actions'] as $a) {
            try {
                if (empty($a['id'])) {
                    continue;
                }

                $stepId = $this->workflowStepIdMap[$a['workflow_step_id']] ?? $a['workflow_step_id'];
                $nextStepId = ! empty($a['next_step_id']) ? ($this->workflowStepIdMap[$a['next_step_id']] ?? $a['next_step_id']) : null;
                $nextWorkflowId = ! empty($a['next_workflow_id']) ? ($this->workflowIdMap[$a['next_workflow_id']] ?? $a['next_workflow_id']) : null;
                $nextWorkflowStepId = ! empty($a['next_workflow_step_id']) ? ($this->workflowStepIdMap[$a['next_workflow_step_id']] ?? $a['next_workflow_step_id']) : null;

                $model = WorkflowStepAction::withTrashed()->firstOrNew(['id' => $a['id']]);
                $model->deleted_at = null;
                $model->forceFill([
                    'workflow_step_id' => $stepId,
                    'action_code' => $a['action_code'] ?? ($a['master_action_code'] ?? null),
                    'next_step_id' => $nextStepId,
                    'next_workflow_id' => $nextWorkflowId,
                    'next_workflow_step_id' => $nextWorkflowStepId,
                    'required_fields' => $a['required_fields'] ?? null,
                    'autofilled_fields' => $a['autofilled_fields'] ?? null,
                    'signing_parties' => $a['signing_parties'] ?? null,
                    'assignee_config' => $a['assignee_config'] ?? null,
                    'transition_config' => $a['transition_config'] ?? null,
                    'alias' => $a['alias'] ?? null,
                    'description' => $a['description'] ?? null,
                    'is_active' => $a['is_active'] ?? true,
                    'deleted_at' => null,
                    'created_by' => $this->adminId,
                    'updated_by' => $this->adminId,
                ])->save();
                $this->counts['workflow_step_actions']++;
            } catch (\Exception $e) {
                Log::warning('Gagal mengimpor WorkflowStepAction ID '.($a['id'] ?? '').': '.$e->getMessage());
            }
        }
    }

    private function importFormTemplatesAndFields(array $data): void
    {
        if (! empty($data['form_templates']) && is_array($data['form_templates'])) {
            $typeMap = ContractType::pluck('id', 'code')->all();
            foreach ($data['form_templates'] as $ft) {
                try {
                    if (empty($ft['id'])) {
                        continue;
                    }
                    $typeId = ! empty($ft['contract_type_code']) ? ($typeMap[$ft['contract_type_code']] ?? null) : null;

                    FormTemplate::withTrashed()->updateOrCreate(
                        ['id' => $ft['id']],
                        [
                            'name' => $ft['name'],
                            'description' => $ft['description'] ?? null,
                            'contract_type_id' => $typeId,
                            'document_type' => $ft['document_type'] ?? config('master.form_template.document_type'),
                            'has_letterhead' => $ft['has_letterhead'] ?? false,
                            'letterhead_json' => $ft['letterhead_json'] ?? null,
                            'is_active' => $ft['is_active'] ?? true,
                            'deleted_at' => null,
                            'created_by' => $this->adminId,
                            'updated_by' => $this->adminId,
                        ],
                    );
                    $this->counts['form_templates']++;
                } catch (\Exception $e) {
                    Log::warning('Gagal mengimpor FormTemplate ID '.($ft['id'] ?? '').': '.$e->getMessage());
                }
            }
        }

        if (! empty($data['form_fields']) && is_array($data['form_fields'])) {
            foreach ($data['form_fields'] as $ff) {
                try {
                    if (empty($ff['id'])) {
                        continue;
                    }
                    FormField::withTrashed()->updateOrCreate(
                        ['id' => $ff['id']],
                        [
                            'form_template_id' => $ff['form_template_id'],
                            'parent_id' => $ff['parent_id'] ?? null,
                            'label' => $ff['label'] ?? '',
                            'name' => $ff['name'],
                            'type' => $ff['type'],
                            'container_type' => $ff['container_type'] ?? null,
                            'placeholder' => $ff['placeholder'] ?? null,
                            'is_required' => $ff['is_required'] ?? false,
                            'use_rich_text' => $ff['use_rich_text'] ?? false,
                            'width' => $ff['width'] ?? '100',
                            'options' => $ff['options'] ?? null,
                            'order' => $ff['order'] ?? 0,
                            'validation_rules' => $ff['validation_rules'] ?? null,
                        ],
                    );
                    $this->counts['form_fields']++;
                } catch (\Exception $e) {
                    Log::warning('Gagal mengimpor FormField ID '.($ff['id'] ?? '').': '.$e->getMessage());
                }
            }
        }
    }

    private function importContractTypes(array $data): void
    {
        if (empty($data['contract_types']) || ! is_array($data['contract_types'])) {
            return;
        }

        $workflowMap = Workflow::pluck('id', 'name')->all();
        foreach ($data['contract_types'] as $t) {
            try {
                if (empty($t['code'])) {
                    continue;
                }
                $workflowId = ! empty($t['workflow_name']) ? ($workflowMap[$t['workflow_name']] ?? null) : null;

                ContractType::withTrashed()->updateOrCreate(
                    ! empty($t['id']) ? ['id' => $t['id']] : ['code' => $t['code']],
                    [
                        'code' => $t['code'],
                        'name' => $t['name'] ?? $t['code'],
                        'workflow_id' => $workflowId,
                        'features' => $t['features'] ?? null,
                        'description' => $t['description'] ?? null,
                        'level' => $t['level'] ?? null,
                        'f1_input_mechanism' => $t['f1_input_mechanism'] ?? config('master.contract_type.input_mechanism'),
                        'f1_form_template_id' => $t['f1_form_template_id'] ?? null,
                        'f1_contract_template_id' => $t['f1_contract_template_id'] ?? null,
                        'f2_input_mechanism' => $t['f2_input_mechanism'] ?? config('master.contract_type.input_mechanism'),
                        'f2_form_template_id' => $t['f2_form_template_id'] ?? null,
                        'f2_contract_template_id' => $t['f2_contract_template_id'] ?? null,
                        'contract_input_mechanism' => $t['contract_input_mechanism'] ?? null,
                        'contract_form_template_id' => $t['contract_form_template_id'] ?? null,
                    ],
                );
                $this->counts['contract_types']++;
            } catch (\Exception $e) {
                Log::warning('Gagal mengimpor ContractType '.($t['code'] ?? '').': '.$e->getMessage());
            }
        }

        $typeMap = ContractType::pluck('id', 'code')->all();
        foreach ($data['contract_types'] as $t) {
            try {
                if (empty($t['code']) || empty($t['parent_code'])) {
                    continue;
                }
                $parentId = $typeMap[$t['parent_code']] ?? null;
                if ($parentId) {
                    ContractType::where('code', $t['code'])->update(['parent_id' => $parentId]);
                }
            } catch (\Exception $e) {
                Log::warning('Gagal mengupdate parent ContractType '.($t['code'] ?? '').': '.$e->getMessage());
            }
        }
    }

    private function importAccessMappings(array $data): void
    {
        if (empty($data['access_mappings']) || ! is_array($data['access_mappings'])) {
            return;
        }

        foreach ($data['access_mappings'] as $am) {
            try {
                $roleId = $this->roleMap[$am['role_name']] ?? null;
                $moduleId = $this->moduleMap[$am['module_identifier']] ?? null;

                $groupId = null;
                if (! empty($am['module_group_name'])) {
                    $groupId = $this->moduleGroupMap[$am['module_group_name']] ?? null;
                } elseif (! empty($am['module_group_id'])) {
                    $groupId = $am['module_group_id'];
                }

                if ($roleId && $moduleId) {
                    AccessModule::withTrashed()->updateOrCreate(
                        ! empty($am['id']) ? ['id' => $am['id']] : ['role_id' => $roleId, 'module_id' => $moduleId],
                        [
                            'role_id' => $roleId,
                            'module_id' => $moduleId,
                            'can_read' => $am['can_read'] ?? false,
                            'can_create' => $am['can_create'] ?? false,
                            'can_update' => $am['can_update'] ?? false,
                            'can_delete' => $am['can_delete'] ?? false,
                            'can_approve' => $am['can_approve'] ?? false,
                            'can_bulk_approve' => $am['can_bulk_approve'] ?? false,
                            'can_bulk_delete' => $am['can_bulk_delete'] ?? false,
                            'module_group_id' => $groupId,
                            'sequence' => $am['sequence'] ?? null,
                        ],
                    );
                    $this->counts['access_mappings']++;
                }
            } catch (\Exception $e) {
                Log::warning('Gagal mengimpor AccessModule: '.$e->getMessage());
            }
        }
    }

    private function importRoleNavigationMappings(array $data): void
    {
        if (empty($data['role_navigation_mappings']) || ! is_array($data['role_navigation_mappings'])) {
            return;
        }

        foreach ($data['role_navigation_mappings'] as $rmg) {
            try {
                $roleId = $this->roleMap[$rmg['role_name']] ?? null;
                $groupId = $this->moduleGroupMap[$rmg['module_group_name']] ?? null;

                if ($roleId && $groupId) {
                    RoleModuleGroup::updateOrCreate(
                        ! empty($rmg['id']) ? ['id' => $rmg['id']] : [
                            'role_id' => $roleId,
                            'module_group_id' => $groupId,
                        ],
                        [
                            'role_id' => $roleId,
                            'module_group_id' => $groupId,
                            'sequence' => $rmg['sequence'] ?? null,
                        ],
                    );

                    if (! empty($rmg['modules']) && is_array($rmg['modules'])) {
                        foreach ($rmg['modules'] as $m) {
                            $moduleId = $this->moduleMap[$m['module_identifier']] ?? null;
                            if ($moduleId) {
                                AccessModule::withTrashed()->updateOrCreate(
                                    [
                                        'role_id' => $roleId,
                                        'module_id' => $moduleId,
                                    ],
                                    [
                                        'can_read' => true,
                                        'module_group_id' => $groupId,
                                        'sequence' => $m['sequence'] ?? null,
                                    ],
                                );
                            }
                        }
                    }

                    $this->counts['role_navigation_mappings']++;
                }
            } catch (\Exception $e) {
                Log::warning('Gagal mengimpor RoleModuleGroup: '.$e->getMessage());
            }
        }
    }

    private function importUsers(array $data): void
    {
        $usersData = null;
        if (! empty($data['users']) && is_array($data['users'])) {
            $usersData = $data['users'];
        } else {
            foreach ($data as $key => $val) {
                if (str_contains($key, 'select * from users') && is_array($val)) {
                    $usersData = $val;
                    break;
                }
            }
        }

        if (empty($usersData) || ! is_array($usersData)) {
            return;
        }

        $validDepartments = array_flip(DB::table('m_departments')->pluck('id')->toArray());
        $validRoles = array_flip(DB::table('m_roles')->pluck('id')->toArray());

        foreach ($usersData as $u) {
            try {
                if (empty($u['name']) && empty($u['username']) && empty($u['email'])) {
                    continue;
                }

                $email = filter_var($u['email'] ?? '', FILTER_VALIDATE_EMAIL) ? $u['email'] : '-';
                $password = $u['password'] ?? bcrypt('Karyawan123!');

                $roleId = null;
                if (! empty($u['role_name'])) {
                    $roleId = Role::whereRaw('lower(name) = ?', [strtolower($u['role_name'])])->value('id');
                } elseif (! empty($u['role_id'])) {
                    $roleId = isset($validRoles[$u['role_id']]) ? $u['role_id'] : null;
                }

                $deptId = null;
                if (! empty($u['department_name'])) {
                    $deptId = Department::whereRaw('lower(name) = ?', [strtolower($u['department_name'])])->value('id');
                } elseif (! empty($u['department_id'])) {
                    $deptId = isset($validDepartments[$u['department_id']]) ? $u['department_id'] : null;
                }

                User::withTrashed()->updateOrCreate(
                    ! empty($u['id']) ? ['id' => $u['id']] : ['username' => $u['username']],
                    [
                        'username' => $u['username'] ?? null,
                        'code' => $u['code'] ?? null,
                        'name' => $u['name'] ?? null,
                        'email' => $email,
                        'password' => $password,
                        'phone_number' => $u['phone_number'] ?? null,
                        'company_id' => ! empty($u['company_code']) ? ($this->companyMap[$u['company_code']] ?? null) : ($u['company_id'] ?? null),
                        'company_group_id' => ! empty($u['company_group_code']) ? ($this->groupMap[$u['company_group_code']] ?? null) : ($u['company_group_id'] ?? null),
                        'department_id' => $deptId,
                        'division_id' => $u['division_id'] ?? null,
                        'region_id' => $u['region_id'] ?? null,
                        'role_id' => $roleId,
                        'is_active' => $u['is_active'] ?? true,
                        'deleted_at' => null,
                        'is_employee' => $u['is_employee'] ?? true,
                    ]
                );
                $this->counts['users']++;
            } catch (\Exception $e) {
                Log::warning('Gagal mengimpor User '.($u['name'] ?? '').': '.$e->getMessage());
            }
        }
    }
}
