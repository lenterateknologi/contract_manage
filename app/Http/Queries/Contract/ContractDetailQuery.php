<?php

namespace App\Http\Queries\Contract;

use App\Models\Contract;
use App\Services\Utils\ShortIdService;
use Illuminate\Database\Eloquent\ModelNotFoundException;
use Illuminate\Support\Str;

class ContractDetailQuery
{
    /**
     * Eager loads for contract detail.
     */
    private const WITH = [
        'creator:id,name,role_id,department_id,division_id,company_id,location_id,idlocation,location_name,company_group_id,region_id,email,spv_id,idreporting_to,reporting_to,nik,jobtitle_name,phone_number,mobile_no',
        'creator.department:id,name',
        'creator.division:id,name',
        'creator.location:id,name,code',
        'creator.company:id,name,company_group_id,region_id',
        'creator.company.companyGroup:id,name',
        'creator.company.region:id,name',
        'creator.supervisor:id,name,role_id,department_id,division_id,company_id,email,nik,jobtitle_name',
        'creator.supervisor.department:id,name',
        'contractType:id,name,parent_id,ancestry_id,f1_input_mechanism,f1_form_template_id,f2_input_mechanism,f2_form_template_id,contract_input_mechanism,contract_form_template_id',
        'contractTypeParent:id,name,f1_input_mechanism,f1_form_template_id,f2_input_mechanism,f2_form_template_id,contract_input_mechanism,contract_form_template_id',
        'submissionType:id,name',
        'approvals.approver:id,name,role_id,department_id,division_id,company_id,location_id,idlocation,location_name,company_group_id,region_id,email,spv_id,idreporting_to,reporting_to,nik,jobtitle_name,phone_number,mobile_no',
        'approvals.approver.department:id,name',
        'approvals.workflowStep:id,step,description,step_category,workflow_id,is_visible,is_active,meta,approver_type,filter_department,filter_company_group,filter_region,filter_company',
        'approvals.workflowStep.workflow:id,name,contract_type_id,workflow_type,meta',
        'approvals.workflowStep.workflow.steps:id,workflow_id,step,description,approver_type,step_category,meta,filter_department,filter_company_group,filter_region,filter_company,is_visible,is_active',
        'approvals.workflowStep.workflow.steps.approverAuthorities.role:id,name',
        'approvals.workflowStep.workflow.steps.approverAuthorities.division:id,name',
        'approvals.workflowStep.workflow.steps.approverAuthorities.department:id,name',
        'approvals.workflowStep.workflow.steps.approverAuthorities.user:id,name',
        'workflow:id,name,contract_type_id,workflow_type,meta',
        'workflow.steps:id,workflow_id,step,description,approver_type,step_category,meta,filter_department,filter_company_group,filter_region,filter_company,is_visible,is_active',
        'workflow.steps.approverAuthorities.role:id,name',
        'workflow.steps.approverAuthorities.division:id,name',
        'workflow.steps.approverAuthorities.department:id,name',
        'workflow.steps.approverAuthorities.user:id,name',
        'originWorkflow:id,name,contract_type_id,workflow_type,meta',
        'originWorkflow.steps:id,workflow_id,step,description,approver_type,step_category,meta,filter_department,filter_company_group,filter_region,filter_company,is_visible,is_active',
        'originWorkflow.steps.approverAuthorities.role:id,name',
        'originWorkflow.steps.approverAuthorities.division:id,name',
        'originWorkflow.steps.approverAuthorities.department:id,name',
        'originWorkflow.steps.approverAuthorities.user:id,name',
        'workflowStep',
        'workflowStep.approverAuthorities.role',
        'workflowStep.approverAuthorities.department',
        'workflowStep.approverAuthorities.division',
        'workflowStep.approverAuthorities.user',
        'workflowStep.approverAuthorities.organizationGroup',
        'workflowStep.workflow:id,name,contract_type_id,workflow_type,meta',
        'workflowStep.actions',
        'versions.uploader:id,name,role_id,department_id,division_id,company_id,email',
        'histories.actor:id,name,role_id,department_id,division_id,company_id,email',
        'messages.user:id,name,role_id,department_id,division_id,company_id,email',
        'attachments.uploader:id,name,role_id,department_id,division_id,company_id,email',
        'formSubmissions:id,contract_id,document_type,form_template_id,current_version,workflow_step_id,step_number,workflow_iteration,submitted_by,updated_at',
        'formSubmissions.submittedBy:id,name,role_id,department_id,division_id,company_id,email',
        'vendor:id,vendor_code,vendor_name,vendor_detail',
        'initiator:id,name,role_id,department_id,division_id,company_id,location_id,idlocation,location_name,company_group_id,region_id,email,spv_id,idreporting_to,reporting_to,nik,jobtitle_name,phone_number,mobile_no',
        'initiator.department:id,name,code,idorg_group,org_group_name',
        'initiator.division:id,name',
        'initiator.location:id,name,code',
        'initiator.company:id,name,company_group_id,region_id',
        'initiator.company.companyGroup:id,name',
        'initiator.company.region:id,name',
        'initiator.supervisor:id,name,role_id,department_id,division_id,company_id,email,nik,jobtitle_name',
        'initiator.supervisor.department:id,name,code,idorg_group,org_group_name',
        'assignedPic:id,name,role_id,department_id,division_id,company_id,location_id,idlocation,location_name,company_group_id,region_id,email,spv_id,idreporting_to,reporting_to,nik,jobtitle_name,phone_number,mobile_no',
        'assignedPic.department:id,name,code,idorg_group,org_group_name',
        'assignedPic.division:id,name',
        'assignedPic.location:id,name,code',
        'assignedPic.company:id,name',
        'assignedPic.supervisor:id,name,role_id,department_id,division_id,company_id,email,nik,jobtitle_name',
        'assignedPic.supervisor.department:id,name,code,idorg_group,org_group_name',
        'assignedBy:id,name,role_id,department_id,division_id,company_id,location_id,idlocation,location_name,company_group_id,region_id,email,spv_id,idreporting_to,reporting_to,nik,jobtitle_name,phone_number,mobile_no',
        'assignedBy.department:id,name,code,idorg_group,org_group_name',
        'meta:contract_id,kop_topik,kop_sub_topik,p1_entity,p1_signer,p1_signer_position,p1_address,p2_entity,p2_signer,p2_signer_position,p2_address,f2_scope,f2_price,f2_payment,f2_tenure,f2_location',
    ];

    /**
     * Selected columns for detail query.
     */
    private const SELECT = [
        'id', 'form_no', 'title', 'description', 'contract_date', 'end_date',
        'contract_type_id', 'transaction_type', 'status', 'current_version',
        'workflow_id', 'origin_workflow_id', 'origin_workflow_step_id', 'workflow_step_id', 'created_by', 'submitted_at',
        'created_at', 'updated_at', 'initiated_by_id', 'vendor_id', 'parent_id',
        'submission_type_id', 'contract_no', 'assigned_pic_id', 'assigned_by_id',
        'received_at', 'assigned_at', 'finished_at', 'closed_at', 'closed_by',
        'contract_type_parent_id', 'contract_type_ancestry_id', 'metadata', 'is_digital_signature',
        'updated_by', 'is_in_sub_workflow', 'branch_step_number', 'current_step_number',
        'current_sub_workflow_id', 'workflow_iteration',
    ];

    /**
     * Get a contract by ID with all relations.
     */
    public function find(string $id): Contract
    {
        $decodedId = ShortIdService::decode($id);

        if (! Str::isUuid($decodedId)) {
            throw (new ModelNotFoundException)->setModel(Contract::class, [$id]);
        }

        return Contract::query()
            ->select(self::SELECT)
            ->with(self::WITH)
            ->findOrFail($decodedId);
    }
}
