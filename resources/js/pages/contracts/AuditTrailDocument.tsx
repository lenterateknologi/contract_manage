                ...
                <div className="mb-8 border-b-2 border-slate-900 pb-8 text-left">
                    ...
                    <div className="grid grid-cols-2 gap-8 pt-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                <div className="mb-1.5 text-[9px] font-semibold tracking-[0.2em] text-slate-400 uppercase">Dicetak Oleh</div>
                                <div className="text-[10px] font-bold text-slate-900 leading-tight">
                                    {printedBy?.name || 'User'}
                                    {printedBy?.id && printedBy.id !== '-' && (
                                        <div className="text-[8.5px] font-mono text-slate-500 font-normal">ID: {printedBy.id}</div>
                                    )}
                                </div>
                            </div>
                        </div>
                ...
                <div className="mt-12 flex items-end justify-between border-t border-slate-100 pt-8">
                    <div className="flex flex-col">
                        ...
                        <span className="text-[10px] font-medium text-slate-700">
                            Dicetak oleh: <strong className="text-slate-900">{printedBy?.name || 'User'}</strong>
                            {printedBy?.id && printedBy.id !== '-' && <span> (User ID: <span className="font-mono">{printedBy.id}</span>)</span>}
                            <span> &bull; {printedBy?.timestamp || formatDateTime(new Date())}</span>
                        </span>
                    </div>
