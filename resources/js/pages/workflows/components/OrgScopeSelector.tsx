...
export default function OrgScopeSelector({ form, companyGroups = [], regions = [], companies = [] }: OrgScopeSelectorProps) {
    // Helper to ensure values are arrays (handles cases where backend returns JSON string or undefined)
    ...
    const companyGroupIds = getSafeArray(form.data.company_group_ids);
    const regionIds = getSafeArray(form.data.region_ids);
    const companyIds = getSafeArray(form.data.company_ids);
    ...
    const filteredCompanies = companies.filter((c: any) => {
        ...
        const matchesRegion =
            regionIds.length === 0 ||
            regionIds.includes(String(c.region_id));
