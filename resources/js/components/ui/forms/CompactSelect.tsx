...
export function CompactSelect({
    ...
    return (
        <div className={cn("space-y-1.5 w-full group bg-card", containerClassName)}>
            ...
            <Popover className="relative w-full">
                {({ open, close }) => (
                    <>
                        <PopoverButton
                            className={cn(
                                "flex h-9 w-full items-center justify-between rounded-lg border bg-white dark:bg-white/[0.02] px-3 text-sm font-medium transition-all outline-none shadow-sm",
                                error
                                ...
                                >
                                    {searchable && (
                                        <div className="flex items-center border-b border-primary/5 dark:border-white/5 p-2">
                                            ...
                                            <input
                                                className="flex h-7 w-full bg-transparent text-xs font-semibold outline-none placeholder:text-primary/10 dark:placeholder:text-white/10 uppercase tracking-wider text-black dark:text-white"
                                                placeholder="Filter..."
                                                value={searchTerm}
                                    ...
                                    <ScrollArea className={cn("max-h-[200px]", filteredOptions.length === 0 && "h-auto")}>
                                        <div className="p-0.5">
                                            {filteredOptions.length === 0 ? (
                                                <div className="py-4 text-center text-xs font-medium text-primary/10 dark:text-white/10 uppercase tracking-widest">
                                                    Kosong
                                                </div>
                                            ) : (
                                                filteredOptions.map((option) => (
                                                    <button
                                                        ...
                                                        className={cn(
                                                            "flex w-full items-center justify-between rounded-md px-2.5 py-2 text-xs font-medium uppercase tracking-tight transition-all text-left",
                                                            option.value === value
