...
// Global cache for animation data to prevent redundant fetches
 ...
 * Uses a global cache for the local loading.json to ensure peak performance
 */
export default function LoadingLottie({ width = 120, height = 120, className }: Props) {
    ...
    useEffect(() => {
        ...
        if (!fetchPromise) {
            fetchPromise = fetch('/assets/lottie/loading.json')
                .then((res) => {
