                ...
                <div key={a.id} className={`flex gap-2.5 ${i < steps.length - 1 ? 'relative pb-4' : ''}`}>
                    ...
                    <div className="flex-1 pt-0.5">
                        <div className="text-foreground text-[12px] font-semibold">
                            {a.role} <span className="text-muted-foreground text-[10px] font-normal">· Seq {a.sequence}</span>
                        </div>
                        ...
                        {a.note && (
                            <div
                                className={`text-muted-foreground bg-muted mt-1.5 rounded border-l-2 px-2.5 py-1.5 text-[11px] ${noteCls[a.status] ?? 'border-l-border'}`}
                            >
                                {a.note}
                            </div>
                        )}
                        <div className="text-muted-foreground mt-1 text-[10px]">{a.approved_at ?? <StatusBadge status={a.status} />}</div>
                    </div>
                </div>
            ))}
        </div>
