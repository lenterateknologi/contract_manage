...
export const InteractiveForm: React.FC<InteractiveFormProps> = ({
    ...
    const pages = useMemo(() => {
        ...
        const estimateHeight = (field: FormField): number => {
            let h = 0;
            const extraMargins = (Number(field.options?.margin_top) || 0) + 
                                (Number(field.options?.margin_bottom) || 0) +
                                (Number(field.options?.spacing_before) || 0) +
                                (Number(field.options?.spacing_after) || 0);

            switch (field.type) {
                case 'kop_surat': h = (field.options?.height || 160) / 3.78; break;
                case 'image': h = (field.options?.height || field.options?.size || 100) / 3.78; break;
                case 'static_text': 
                ...
                case 'date':
                case 'labeled_value':
                    h = 10.6; break; // Balanced for 2-element jump
                case 'textarea': h = 30; break;
                case 'signature_box': h = 40; break;
                case 'group':
                case 'grid_x':
                case 'grid_y':
            ...
            <SortableContext items={allFieldIds} strategy={verticalListSortingStrategy}>
                {pages.map((pageFields, idx) => (
                    <Page 
                        ...
                        className={isBuilder ? 'hover:ring-2 hover:ring-primary/20' : ''}
                    >
                        {isBuilder && pages.length === 1 && pageFields.length === 0 && (
                            <div className="flex h-[200mm] w-full flex-col items-center justify-center gap-4 rounded-2xl border-4 border-dashed border-slate-100 bg-slate-50/50">
                                ...
                                <div className="text-center">
                                    <h3 className="text-sm font-black tracking-widest text-slate-400 uppercase">Halaman Kosong</h3>
